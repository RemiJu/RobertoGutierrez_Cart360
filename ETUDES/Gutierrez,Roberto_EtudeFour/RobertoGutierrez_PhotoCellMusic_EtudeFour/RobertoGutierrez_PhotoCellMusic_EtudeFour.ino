...
void live()
{
   ...
   if(analogRead(NOTE_IN_PIN)){
    tone(BUZZER_PIN, analogRead(NOTE_IN_PIN)+ offsetFrequency, 100); //If this part of the code was functional last  
   }                                                                 //time It should remain so this time as well
   // output sound
...
void recordWithDuration()
{
  ...
  if (countNotes < MAX_NOTES)
  {
    ...
    if (testNote > theAdjuster && (timePassed < MAX_PRESS_TIME)) {
      ...
      */
      ...
      /*** FUNCTION TO IMPLEMENT ***/
        startUpTimer();
        activeNoteButton = true; //Other modes wouldn't work so a classmate let me see
        //their code, and this seemed different, I didn't add this in mine.
    ...
    else if (testNote > theAdjuster && (timePassed > MAX_PRESS_TIME))
    {
      ...
      //tone(BUZZER_PIN, 0, 0);
      noTone(BUZZER_PIN); //wow, there was just a preset function.
    }
     /*** STATE C::: *******************************************
   ...
   //I'm not super clear on these instructions? I take them to mean that I shouldn't
   //change things here, other than to implement the new functions below. Might
   //come back here if it turns out I need to do things.
  }
...
void startUpTimer()
{
  //Use Millis to make a timer
  startTime = millis();
  //records the current millis value as start time.
  
  //I don't remember if there's a way to reset millis count without weird
  //side effects on the rest of the code, if it was even possible in unity.
  //Millis just keeps conting up. So to determine an actual passage of recorded
  //time another variable would need to be set up to reset and count up. 
  timePassed = 0;
...
void updateTimer()
{
 ...
 timePassed = millis() - startTime; //Millis keeps going so to determine actual time passed the millis
 //recorded at startTime is subtracted from the current millis count, resulting i
 //a value befitting our needs.
...
void playCurrentNote()
{
  ...
  //Same as live play? but while using the testnote variable to store the note?
  if (analogRead(NOTE_IN_PIN)){

   tone(BUZZER_PIN, analogRead(NOTE_IN_PIN) + offsetFrequency, 100);


   //testNote = analogRead(NOTE_IN_PIN) + offsetFrequency;
  }
...
{
 //Put in the Note like in etude 3 (that kind of worked last time right
 notes[countNotes] = analogRead(NOTE_IN_PIN) + offsetFrequency;
 //uhhhh, timing works similarly I assume thats pat of what the new durations 
 //array is for?
 ...
 //a friend told me I should reset active button, though honestly I'm not sure 
 //exactly why. but timePassed I will also be reset for the next note.
...
{
  //Thankfully this part is simple, the only part I didn't freeze up on immediately
  //but being so simple to figure out makes me almost feel like I may hav made a mistake
  if(analogRead(PHOTO_PIN)){
   return  analogRead(PHOTO_PIN) * 2.5;//wasn't sure what scale accordingly would be so just random numbers for testing, may change
...
{
 //friend told me a bunch of random stuff which only confused me more haha
 //decided to look through the YET MORE ARDUINO SLIDES as was recommended in class
runningAverageBuffer[nextCount] = analogRead(PHOTO_PIN);
//pretty much same situation as prior arrays
nextCount++;
...
activeFrequency = 0; //using the global variable

for(int i = 0; i < RUNNING_SAMPLES; i++){
  activeFrequency += runningAverageBuffer[i];
}
 averageOffsetFreq = activeFrequency/RUNNING_SAMPLES;
...
void colorLED(int col)
{
  ...
  //????? I don't undertand what I was supposed to implement here because colour led 
  //was already ade before, and it looks fine here?
...
{
  //This is like one of those tests where it tells you to read all instructions before
  //working and then it turns out the last instruction is don't do the test.
  //Thats how I feel about the ColorLED function and how i didnt know what to do
  
  for(int i = 0; i < countNotes; i++){ //these kind of i code always confuses me when my frien tells
  //me to use them, and also when I look at them in references online. I can only assume it creates
  //an integer for use in the function 
  tone(BUZZER_PIN, notes[i], durations[i]); //like here where its used to keep count

  delay(durations[i]);

  //Heere is where the colorLEDcomes in. Where I have to use a map to make it work. 
  //I find maps a little odd for some reason. probably a little trauma from the issues 
  //they caused us in the THIS assignment, with the colour sensor.
  
  //Sorry about the weird train of thought comment. I know this isn't how they should be 
  //usually but this is how I keep myself focused, otherwise I'd get overwhelmed staring 
  //at this code and try to find something else to do, distracting me even more.

  int duRange = analogRead(durations[i]); // duration's range 
  int colouRange = map(duRange, 0, 5000, 0, 255); // to colour Range
  
  colorLED(colouRange);//I don't know if theres a way to Map without creating new ints like the etude says not
  //to... so I guess I will do it.
